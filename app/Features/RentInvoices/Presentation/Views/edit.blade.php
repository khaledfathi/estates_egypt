...
@section('title', 'سجل الايجارات |تحديث فاتورة سداد ايجار ')
@section('active-estates', 'active')
@section('breadcrumbs')
    @isset($unitContract)
        {{ Diglactic\Breadcrumbs\Breadcrumbs::render('estates.units.contracts.rent-invoices.edit', $estate->id, $unit->id, $unitContract->id, $rentInvoice->id) }}
    @endisset
@endsection
@section('content')
    ...
    <div class="container-fluid">
        @if (isset($unitContract))
            ...
            {{-- edit rent invoice form --}}
            <div class="row" style="display:flex; justify-content: center;">
                <form id="form" class="col-sm-12 col-md-10 col-lg-6" method="post"
                    action="{{ route('estates.units.contracts.rent-invoices.update', ['estate' => $estate->id, 'unit' => $unit->id, 'contract' => $unitContract->id, 'rent_invoice' => $rentInvoice->id]) }}"
                    enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    
                    <input type="hidden" name="contract_id" value="{{ $unitContract->id }}">
                    <input type="hidden" name="transaction_id" value="{{ $rentInvoice->transaction->id }}">
                    <div class="card">
                        <div class="card-header">
                            <strong>تحديث فاتورة سداد ايجار</strong>
                        </div>
                        <div class="card-block">
                            ...
                            <div class="form-group">
                                ...
                                <input name="amount" type="text" class="form-control" id="amount"
                                    placeholder="قيمة الايجار او جزء منها "
                                    value="{{ old('amount', $rentInvoice->transaction->amount) }}">
                            </div>
                            {{-- for month --}}
                            <div class="form-group">
                                ...
                                <select name="for_month" type="text" class="form-control" id="for_month">
                                    @foreach ($months as $month)
                                        <option value="{{ $month->value }}"
                                            {{ old('for_month', $rentInvoice->forMonth) == $month->value ? 'selected' : null }}>
                                            {{ $month->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input name="for_year" type="number" class="form-control" id="for_year"
                                    placeholder="العام" value="{{ old('for_year', $rentInvoice->forYear) }}">
                            </div>
                            {{-- / for year --}}
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="notes" class="form-control" id="notes" placeholder="اى ملاحظات او تفاصيل اخرى عن المالك">{{ old('notes', $rentInvoice->transaction->description) }}</textarea>
                            </div>
                            {{--  / notes --}}
